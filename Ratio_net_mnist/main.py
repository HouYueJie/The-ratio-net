...
class the_net():
    def __init__(self,train_config,stru_config,name,loggin):
        ...
        if self.net_name=='MLP':
            self.activate_method=self.stru_config['struc'][0][1]
        elif self.net_name=='RBF':
            self.activate_method='RBF'
        else:
            self.model_method=str(self.stru_config['up_order'])+'_'+str(self.stru_config['down_order'])+'_'+self.stru_config['flag']
            self.activate_method='Pade'
        
        self.D=give_batch(self.activate_method)
    ...
    def build_net(self):
        self.target=tf.placeholder(tf.float32, [None, self.D.output_])
        if self.net_name=="MLP":
            self.y = N.MLP(self.stru_config,self.D.input_,self.D.output_)
            ...
            self.y = N.Pade(self.stru_config,self.D.input_,self.D.output_)
            variables=self.y.get_trainable_variables()
            print("当前%s模型参数:\n %s\n"%(self.net_name,variables))
            show_parameter_count(self.loggin,variables)
            
        elif self.net_name=="RBF":
            self.y = N.RBF(self.stru_config,self.D.input_,self.D.output_)
            variables=self.y.get_trainable_variables()
            print("当前%s模型参数:\n %s\n"%(self.net_name,variables))
            show_parameter_count(self.loggin,variables)
            
        else:
            print("网络模型错误！请选择（MLP/Pade/RBF）")
            exit()
    ...
    #initialize model (restore ckpt or init model)
    def initialize(self):
    ...
    def train(self):
        ...
        for self.epoch in range(self.max_iter):
            print("+++++++++++++++++++++++++++++++++++++++++++++train epoch %s of total %s epoches+++++++++++++++++++++++++++++++++++++++++++++"%(self.epoch,self.max_iter))
            self.loggin.info("train epoch %s of total %s epoches"%(self.epoch,self.max_iter))
            for step in range(self.step_each_iter):
                ...
                if (step+1)%self.step_show_train==0:
                    ...
                    print("train->loss %.4f, in epoch %s, in global step %s,\
                    acc is %s, taks %s seconds"%(loss,self.epoch,gs,acc,time.time()-st))
                    self.loggin.info("train->loss %s, in epoch %s, in global step %s, acc is %s,\
                    taks %s seconds"%(loss,self.epoch,gs,acc,time.time()-st))
                    st=time.time()
                ...
                if (step+1)%self.step_show_test==0:
                    ...
                    loss,logit=self.sess.run([self.loss,self.y.value],\
                                        feed_dict={self.y.input:feature,self.target:label})
                    acc=self.cal_acc(logit,label)
                    print("test->loss %.4f, in epoch %s, in global step %s,\
                    acc is %s, taks %s seconds\n"%(loss,self.epoch,gs,acc,time.time()-st))
                    self.loggin.info("test->loss %s, in epoch %s, in global step %s, \
                    acc is %s, taks %s seconds\n"%(loss,self.epoch,gs,acc,time.time()-st))
                    st=time.time()

            #save model
            if (self.epoch+1)%self.epoch_save==0:
                #修改了保存路径
                self.saver.save(self.sess, self.ckpt_save_path+"/check.ckpt")
                print("Model saved in path: %s in epoch %s. acc is %s, loss is %s\n" % (self.ckpt_save_path,self.epoch,acc,loss))
                self.loggin.info("Model saved in path: %s in epoch %s. acc is %s, loss is %s\n" % (self.ckpt_save_path,self.epoch,acc,loss))
...
if __name__ == '__main__':
    ...
    if which=="Pade":
        ...
        handler = logging.FileHandler(r"logging/the_ratio_net_%s_log"%name)
        handler.setLevel(logging.INFO)
        formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
        handler.setFormatter(formatter)
        logger.addHandler(handler)
